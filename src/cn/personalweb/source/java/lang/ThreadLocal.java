 ...
 * 此类提供线程局部变量。这些变量与普通变量不同，每个访问一个线程（通过其get或set方法）的线程都有其自己的，独立初始化的变量副本。
 ...
 *
 * 通常情况下，我们创建的变量是可以被任何一个线程访问并修改的。而使用ThreadLocal创建的变量只能被 当前线程访问 ，其他线程则无法访问和修改。
 *
    ...
    static class ThreadLocalMap {
        ...
        private Entry getEntryAfterMiss(ThreadLocal<?> key, int i, Entry e) {
            ...
            while (e != null) {
                ...
                if (k == key)
                    return e;
                if (k == null)
                    // key 为 null说明已经被回收, 把 key 对应的 value 值给回收然后替换成新的值
                    expungeStaleEntry(i);
                else
                    i = nextIndex(i, len);
                e = tab[i];
        ...
        private void set(ThreadLocal<?> key, Object value) {
            ...
            for (Entry e = tab[i];
                ...
                if (k == null) {
                    // key 为 null 说明这个 key 已经被回收了,这个 entry 无用, 可把 value 代替,并重新设置 key 值
